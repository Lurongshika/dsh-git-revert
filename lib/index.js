 ...
 * the start of every agent turn, and exposes one same-origin HTTP endpoint
 * `/api/git-revert` that the browser half calls to restore the workspace to the
 * state before a given turn. Uses the `shell` executor and the `webServer`
...
function apply(ctx) {
  ...
  async function doRevert(sessionId, turn) {
    ...
    if (!snap) return { ok: false, message: "没有该轮对话前的快照（插件可能在对话中途才启用）" };
  ...
  // Snapshot at the first step of every turn (turn/step both start at 1).
