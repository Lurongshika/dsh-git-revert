		...
		function RevertButton(props) {
			...
			const onClick = () => {
				...
				fetch("/api/git-revert", {
					...
					setStage("idle");
					setMessage(data && data.message ? String(data.message) : "已回溯");
				}).catch((err) => {
					setStage("idle");
					setMessage("回溯失败：" + (err && err.message ? err.message : String(err)));
				});
			};

			const label = stage === "busy" ? "回溯中…" : (stage === "confirm" ? "确认回溯？" : "版本回溯");
